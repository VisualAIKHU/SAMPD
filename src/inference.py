...
def val_epoch(model: SSD300_3Way, dataloader: DataLoader, input_size: Tuple, inference_func, min_score: float = 0.1) -> Dict:
    model.eval()
    ...
    with torch.no_grad():
        for i, blob in enumerate(dataloader):
            image_vis, image_lwir, _, _, _, _,_, _,_, _, indices,_,_ = blob

            image_vis = image_vis.to(device)
            image_lwir = image_lwir.to(device)

            # Forward prop.
            predicted_locs, predicted_scores, _, _, _, _, _, _, _ = model(image_vis, image_lwir)  # (N, 8732, 4), (N, 8732, n_classes)

            # Detect objects in SSD output
            detections = inference_func(predicted_locs, predicted_scores, min_score=min_score, max_overlap=0.425, top_k=200)
...
def run_inference(model_path: str, fdz_case: str) -> Dict:
    ...
    try:
        inference_func = model.detect_objects_cuda
    except:
        inference_func = model.module.detect_objects_cuda
    ...
    results = val_epoch(model, test_loader, input_size, inference_func)
    return results
...
if __name__ == '__main__':
    ...
    conf_name = arguments.model_path.split("/")[2]
    arguments.result_dir = opj(arguments.result_dir, conf_name)
    ...
    # Visualizing
    if arguments.vis:
        vis_dir = opj(arguments.result_dir, model_path)
