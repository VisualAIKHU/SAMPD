...
class VGGBase3Way(VGGBase):
    ...
    def forward(self, image_vis, image_lwir):
        ...
        conv4_3_feats = conv4_3_feats / norm
        conv4_3_feats = conv4_3_feats * self.rescale_factors
        #self.conv4_3_feats = conv4_3_feats
...
class SSD300_3Way(SSD300):
    ...
    def __init__(self, n_classes):
        ...
        self.priors_cxcy, self.priors_cxcy_3way = self.create_prior_boxes()
    ...
    def create_prior_boxes(self):
        ...
        prior_boxes = torch.FloatTensor(prior_boxes).to(device)  # (8732, 4)
        prior_boxes_3way = torch.cat([prior_boxes, prior_boxes, prior_boxes], dim=0)  # Duplicate and concatenate (8732 * 3, 4)

        return prior_boxes, prior_boxes_3way
    ...
    def detect_objects_cuda_3way(self, predicted_locs, predicted_scores, min_score, max_overlap, top_k, mode):
        """
        Decipher the 8732 locations and class scores (output of ths SSD300) to detect objects.
        For each class, perform Non-Maximum Suppression (NMS) on boxes that are above a minimum threshold.
        """
        batch_size = predicted_locs.size(0)
        priors = self.priors_cxcy_3way if mode == "3way" else self.priors_cxcy
        ...
        for i in range(batch_size):
            # Decode object coordinates from the form we regressed predicted boxes to
            decoded_locs = cxcy_to_xy(gcxgcy_to_cxcy(predicted_locs[i], priors)) 
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predicted_locs, predicted_scores, boxes, labels, cos_weights=None):
        ...
        if true_locs[positive_priors].shape[0] == 0:
            ...
            loc_loss = self.smooth_l1(predicted_locs[positive_priors], true_locs[positive_priors])  # (total_positive_priors, 4)
